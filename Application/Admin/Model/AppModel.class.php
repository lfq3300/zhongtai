...
class AppModel extends CommonModel
{
    ...
    public function getEffeList(){
        ...
        $appList = M()->query(" SELECT id,appid,authorizer_refresh_token,verify_type_info FROM mc_app WHERE  verify_type_info = 0 AND day_synchron = 2 AND synchron <> 3 ORDER BY id DESC limit 0,3");
        return $appList;
    }

    public function getHisList(){
        //此处不能做缓存 synchron  = 2 的状态会更新 所以不能缓存 且未同步的
        $appList = M()->query("SELECT id,appid,authorizer_refresh_token,verify_type_info FROM mc_app WHERE  verify_type_info = 0 AND synchron = 1 ORDER BY id DESC limit 0,3");
        // 查询 全部公众号 然后请求 公众号数据  必须通过微信公众号认证  获取用户增长的话
        return $appList;
    }
